...
//
// @file ex_ellipse_2d.cpp
// @brief Reads an ellipse description, creates a VMM domain, and exports VTK.
...
int main()
{
    try {
        ...
        const auto data_file = source_dir / "geometry_data" / "ellipse_2d.yaml";
        const auto output_file = source_dir / "output_data" / "ellipse_2d.vtk";
        const auto values = read_scalar_yaml(data_file, "ex_ellipse_2d");

        const vmm::domain::Point2D centre{
            value_or(values, "ellipse.centre_x", 0.0),
            value_or(values, "ellipse.centre_y", 0.0)
        };
        auto domain = vmm::domain::make_ellipse_2d(value_or(values, "ellipse.radius_x", 0.0),
                                                   value_or(values, "ellipse.radius_y", 0.0),
                                                   centre,
                                                   static_cast<std::size_t>(value_or(values, "ellipse.segments", 96.0)));
        ...
        vmm::io::write_legacy_vtk_polygonal_domain_2d(output_file, "Ellipse 2D geometry", domain);

        std::cout << "Ellipse 2D example\n";
        std::cout << "data_file: " << data_file << '\n';
        std::cout << "vertices: " << domain.exterior().vertex_count() << '\n';
        std::cout << "area: " << domain.area() << '\n';
        std::cout << "vtk_file: " << output_file << '\n';
    } catch(const vmm::error::MeshException& exception) {
        std::cerr << "ex_ellipse_2d failed: " << exception.what() << '\n';
        return 1;
    }
    return 0;
}
