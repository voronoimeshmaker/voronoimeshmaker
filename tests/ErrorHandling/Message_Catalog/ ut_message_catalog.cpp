// -----------------------------------------------------------------------------
// ut_message_catalog.cpp
// Parallel tests for message rendering (EN/PT) using explicit locale and
// under concurrent config toggling. Focus: thread-safety of render().
// One test executable dedicated to MessageCatalog.
...
TEST(MessageCatalog, Parallel_Render_ExplicitLocale_NoGlobalState) {
    ...
    for (int t = 0; t < T; ++t) {
        ths.emplace_back([&, t]{
            ...
            ve::Language lang = (t % 2 == 0) ? ve::Language::EnUS : ve::Language::PtBR;

            for (int i = 0; i < M; ++i) {
                std::string val = "t" + std::to_string(t) + "-" + std::to_string(i);
                std::string msg = ve::render(ve::CoreErr::InvalidArgument, lang,
                                             {{"name", val}});
    ...
    EXPECT_EQ(failures.load(), 0) << "render() failed to substitute placeholders";
}
...
TEST(MessageCatalog, Parallel_Render_ConfigDriven_WithLanguageToggling) {
    ...
    for (int r = 0; r < Readers; ++r) {
        readers.emplace_back([&]{
            ...
            for (int i = 0; i < Iters; ++i) {
                ...
                if (!cfg) { failures++; break; }
                auto lang = cfg->language;
                std::string val = "R" + std::to_string(r) + "-" + std::to_string(i);
                std::string msg = ve::render(ve::CoreErr::InvalidArgument, lang,
                                             {{"name", val}});
                if (msg.empty() || !contains(msg, val)) {
                    failures.fetch_add(1, std::memory_order_relaxed);
    ...
    EXPECT_EQ(failures.load(), 0) << "render() failed under config toggling";
}
