...
FiniteVolumeMesh3D make_mesh()
{
    const std::vector<Real> z_interfaces{0.0, 2.0, 5.0};
    return extrude_finite_volume_mesh_3d(make_two_square_mesh(), z_interfaces);
}
...
bool contains_string(const std::vector<std::string>& values, const std::string& expected)
{
...
TEST(MohidNGHDF5Writer3DTest, WritesMohidNG3DSchemaPackage)
{
    const auto mesh = make_mesh();
    const auto output = std::filesystem::temp_directory_path() / "vmm_tst_mohidng_writer_3d.mngpkg.h5";

    vmm::io::MohidNGHDF5Writer3DOptions options;
    options.coordinate_reference_system = "EPSG:4326";
    options.note = "writer 3D schema test";
    options.vertical_reference = "chart datum";
    vmm::io::write_mohidng_hdf5_mesh_3d(output, mesh, options);

    const auto file = open_file(output);
    ASSERT_TRUE(file.valid());
    EXPECT_EQ(read_string_attribute(file.id(), "schema_version"), vmm::io::kMohidNGVoronoiMeshPackage3DSchemaVersion);
    ...
    EXPECT_EQ(read_string_attribute(file.id(), "vertical_reference"), "chart datum");
    EXPECT_EQ(read_string_attribute(file.id(), "note"), "writer 3D schema test");
    EXPECT_EQ(read_int_attribute(file.id(), "dimension"), 3);

    EXPECT_EQ(dataset_dimensions(file.id(), "/nodes/id"), std::vector<hsize_t>{mesh.node_count()});
    EXPECT_EQ(dataset_dimensions(file.id(), "/nodes/xyz"), (std::vector<hsize_t>{mesh.node_count(), 3U}));
    EXPECT_EQ(dataset_dimensions(file.id(), "/cells/id"), std::vector<hsize_t>{mesh.cell_count()});
    EXPECT_EQ(dataset_dimensions(file.id(), "/cells/centre"), (std::vector<hsize_t>{mesh.cell_count(), 3U}));
    EXPECT_EQ(dataset_dimensions(file.id(), "/cells/volume"), std::vector<hsize_t>{mesh.cell_count()});
    EXPECT_EQ(dataset_dimensions(file.id(), "/faces/id"), std::vector<hsize_t>{mesh.face_count()});
    EXPECT_EQ(dataset_dimensions(file.id(), "/faces/centre"), (std::vector<hsize_t>{mesh.face_count(), 3U}));
    EXPECT_EQ(dataset_dimensions(file.id(), "/faces/unit_normal"), (std::vector<hsize_t>{mesh.face_count(), 3U}));
    EXPECT_EQ(dataset_dimensions(file.id(), "/faces/area"), std::vector<hsize_t>{mesh.face_count()});
    EXPECT_EQ(dataset_dimensions(file.id(), "/faces/role"), std::vector<hsize_t>{mesh.face_count()});
    EXPECT_EQ(dataset_dimensions(file.id(), "/boundary_patches/id"), std::vector<hsize_t>{mesh.patch_count()});
    EXPECT_EQ(dataset_dimensions(file.id(), "/connectivity/cell_face_offset"), std::vector<hsize_t>{mesh.cell_count()});
    EXPECT_EQ(dataset_dimensions(file.id(), "/connectivity/cell_face_count"), std::vector<hsize_t>{mesh.cell_count()});
    EXPECT_EQ(dataset_dimensions(file.id(), "/connectivity/cell_face_id"), std::vector<hsize_t>{mesh.cell_faces.size()});
    EXPECT_EQ(dataset_dimensions(file.id(), "/connectivity/face_node_offset"), std::vector<hsize_t>{mesh.face_count()});
    EXPECT_EQ(dataset_dimensions(file.id(), "/connectivity/face_node_count"), std::vector<hsize_t>{mesh.face_count()});
    EXPECT_EQ(dataset_dimensions(file.id(), "/connectivity/face_node_id"), std::vector<hsize_t>{mesh.face_nodes.size()});

    const auto volumes = read_double_dataset(file.id(), "/cells/volume");
    ASSERT_EQ(volumes.size(), mesh.cell_count());
    EXPECT_DOUBLE_EQ(volumes[0], 2.0);
    EXPECT_DOUBLE_EQ(volumes[1], 2.0);
    EXPECT_DOUBLE_EQ(volumes[2], 3.0);
    EXPECT_DOUBLE_EQ(volumes[3], 3.0);

    const auto roles = read_string_dataset(file.id(), "/faces/role");
    ASSERT_EQ(roles.size(), mesh.face_count());
    EXPECT_TRUE(contains_string(roles, "lateral"));
    EXPECT_TRUE(contains_string(roles, "bottom"));
    EXPECT_TRUE(contains_string(roles, "top"));
    EXPECT_TRUE(contains_string(roles, "internal-horizontal"));

    const auto cell_faces = read_int64_dataset(file.id(), "/connectivity/cell_face_id");
    ASSERT_EQ(cell_faces.size(), mesh.cell_faces.size());
    EXPECT_EQ(cell_faces.front(), static_cast<std::int64_t>(mesh.cell_faces.face_ids.front().value));
    EXPECT_EQ(cell_faces.back(), static_cast<std::int64_t>(mesh.cell_faces.face_ids.back().value));
}

TEST(MohidNGHDF5Writer3DTest, RejectsInvalidMeshBeforeExport)
{
    auto mesh = make_mesh();
    mesh.cells.volume[0] = 0.0;
    const auto output = std::filesystem::temp_directory_path() / "vmm_tst_mohidng_writer_3d_invalid.mngpkg.h5";

    EXPECT_THROW(vmm::io::write_mohidng_hdf5_mesh_3d(output, mesh), vmm::error::MeshException);
}
