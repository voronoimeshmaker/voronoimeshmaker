...
class PrintLine {
public:
    explicit PrintLine(unsigned _lsize = 80) : m_lsize(_lsize) {}
...
public:
     ...
     */
    HeaderPrinter(std::string_view _title, 
                 std::string_view _author, 
                 std::string_view _version, 
                 std::string_view _description,
                 std::string_view _licenseText = "",
                 unsigned _lineSize = 80)
        : m_title(_title), m_author(_author), m_version(_version), 
          m_description(_description), m_licenseText(_licenseText),
          m_lineSize(_lineSize), m_useDefaultLicense(_licenseText.empty()) {}
    
    friend std::ostream& operator<<(std::ostream& _os, const HeaderPrinter& _hp) {
        // Linha superior
        _os << "//";
    ...
    HeaderPrinter& lineSize(unsigned size) {
        m_lineSize = size;
        return *this;
    }
...
private:
    void printCentered(std::ostream& _os, std::string_view _text) const {
