...
#define VMM_ERROR(err_enum, /* {{"k","v"},...} */ ...)                           \
    do {                                                                         \
        using ::vm::error::Config;                               \
        using ::vm::error::ErrorRecord;                          \
        using ::vm::error::Language;                             \
        using ::vm::error::MessageCatalog::render;               \
        using ::vm::error::ErrorTraits;                          \
        using ::vm::error::Severity;                             \
        using ::vm::error::error_code;                           \
        auto e = (err_enum);                                                     \
        auto cfg = Config::get();                                                \
        ErrorRecord rec;                                                         \
        ...
        ::vm::error::ErrorManager::log(std::move(rec));          \
    } while (0)
...
#  define VMM_ASSERT(cond)                                                       \
    do {                                                                         \
        if (!(cond)) {                                                           \
            VMM_THROW(::vm::error::CoreErr::AssertFailed);      \
        }                                                                        \
    } while (0)
#else
